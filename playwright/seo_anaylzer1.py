...
parser.add_argument("--cwd_flag", type=str)
...
base_path = os.getcwd()
if int(args.cwd_flag) == USE_ENV_CWD:
    base_path = os.getenv("CWD_LARAVEL")
...
async def trigger_tests(url, primaryKeyword):
    ...
    async with async_playwright() as p:
        ...
        if doesFileExist:
            ...
            html_file_name, html_content, trace_network = extract_html_file(f"{args.scraped_url_id}.zip", base_path)
        ...
        extract_words_from_body(f"{args.scraped_url_id}.zip", base_path)
        breadcrumb_res = breadcrumb.check_breadcrumbs(html_content)
        file_size = await check_file_size(f"{args.scraped_url_id}.zip", base_path)
        ...
        protocol, valid_till, new_protocol_version = extract_ssl_details(f"{args.scraped_url_id}.zip", base_path)

        network_array = extract_network_content(f"{args.scraped_url_id}.zip", base_path)
        network_payload = {
